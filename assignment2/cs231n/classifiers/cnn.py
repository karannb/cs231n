...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        #H_f = int(H_/2)
        #W_f = int(W_/2)
        #SAME conv and 2x2 MAXPOOL, acc to fwd. pass given below.
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, dAfinal = softmax_loss(scores, y)
        
        loss += self.reg*0.5*(np.sum(W1*W1) + np.sum(W2*W2) + np.sum(W3*W3))
        
        dA_prev, grads["W3"], grads["b3"] = affine_backward(dAfinal, fc2_cache)
        dA_prev, grads["W2"], grads["b2"] = affine_relu_backward(dA_prev, fc1_cache)
        dA_prev, grads["W1"], grads["b1"] = conv_relu_pool_backward(dA_prev, conv_cache)
