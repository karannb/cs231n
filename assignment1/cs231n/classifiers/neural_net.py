...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    layer1_A = np.maximum(0, layer1_Z)
    scores = np.dot(layer1_A, W2) + b2
    layer2_Z = scores
    ...
    loss = -np.sum(np.log(scores_[range(N),y]))
    loss /= N
    ...
    dlayer1_A = np.dot(Dscore, W2.T)
    IZ = (layer1_Z>0).astype(float)
    dlayer1_Z = dlayer1_A*IZ
    
    grads['W1'] = np.dot(X.T,dlayer1_Z)/N + 2*reg*W1
    
    grads['b1'] = np.sum(dlayer1_Z, axis = 0)/N
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def predict(self, X):
    ...
    scores = layer2_Z
    scores -= np.max(layer2_Z, axis = 1)[:, np.newaxis]
    exp_scores = np.exp(scores)
    sum_exp = np.sum(exp_scores, axis = 1)
    scores_ = exp_scores/sum_exp[:, np.newaxis]
    
    layer2_A = scores_
    y_pred = np.argmax(layer2_A, axis = 1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
