...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_r = torch.exp(sim(z_k, z_k_N)/tau)
        sim_s1, sim_s2 = 0, 0
        for i in range(2*N):
          
          if(k != i):

            sim_s1 += torch.exp(sim(z_k, out[i])/tau)

          if(k + N != i):

            sim_s2 += torch.exp(sim(z_k_N, out[i])/tau)
          
        total_loss += -torch.log(sim_r/sim_s1) -torch.log(sim_r/sim_s2)        
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left/torch.linalg.norm(out_left, dim=1, keepdim=True))* out_right/(torch.linalg.norm(out_right, dim=1, keepdim=True))
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, out.T)/torch.matmul(torch.linalg.norm(out, axis=1, keepdim=True), torch.linalg.norm(out.T, axis=0, keepdim=True))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, axis=1).to(device)
    ...
    loss = torch.mean(-torch.log(numerator/denom)).to(device)
