...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
      ...
      output_scores = model(X_fooling)[0]
      
      if torch.argmax(output_scores) == target_y:
        print("DONE")
        break
      
      if (i%10) == 0:
        print(output_scores[target_y], torch.max(output_scores))

      loss = output_scores[target_y]
      loss.backward()
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    scores = model(img)[0, target_y] - l2_reg*torch.sum(img**2)
    scores.backward()
