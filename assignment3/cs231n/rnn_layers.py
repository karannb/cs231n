...
def rnn_step_backward(dnext_h, cache):
    ...
    xt, prev_ht, Wxt, Wht, next_ht = cache
    dt = 1-(next_ht**2)
    dWx = np.dot(xt.T, dnext_h*dt)
    dWh = np.dot(prev_ht.T, dnext_h*dt)
    dprev_h = np.dot(dnext_h*dt, Wht.T)
    dx = dx = np.dot(dnext_h*dt, Wxt.T)
    db = np.sum(dnext_h*dt, axis = 0)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (np.tanh(next_c), o, f, prev_c, g, i, x, Wx, prev_h, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    a_c, o, f, prev_c, g, i, x, Wx, prev_h, Wh = cache

    dnext_c += dnext_h*o*(1-a_c**2)
    do = dnext_h*a_c
    ...
    #print(da.shape, activation.shape)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    N, H = h0.shape
    cell_state = np.zeros_like(h0)
    h = np.zeros((N, T, H))
    cache = []
    prev_h = h0
    for t in range(T):
      h[:, t, :], cell_state, cache_t = lstm_step_forward(x[:, t, :], prev_h, cell_state, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dx[:, t, :], dprev_h, dnext_c, dWx_t, dWh_t, db_t = lstm_step_backward(dprev_h + dh[:, t, :], dnext_c, cache[t])
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
...
def temporal_affine_forward(x, w, b):
    ...
    N, T, D = x.shape
    M = b.shape[0]
    out = x.dot(w) + b #.reshape(N * T, D), .reshape(N, T, M) not needed
    cache = x, w, b, out
    return out, cache
